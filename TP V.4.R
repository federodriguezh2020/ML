...
train_borrador3 <- train %>% filter(age == 23 & TARGET == 1)
...
valores.m = c(60,70,80,90,100,120)  # valores de "m", numero de variables candidatas para corte
valores.maxnode = c(32, 34, 36, 40)  # Complejidad de Arboles en el bosque.
...
svm.rad$accuracies # Tasas de acierto en cada fold

svm.rad$tot.accuracy # Tasas de acierto estimada por VC
...
reg.gbm = gbm(TARGET~.,
              data=train, 
              distribution="bernoulli", # 'bernoulli' (clasificaci?n)
              n.trees=50,            # M (overfitting)
              shrinkage=.01,           # lambda (regularizaci?n)
              interaction.depth=2,     # Complejidad de cada ?rbol.
              n.minobsinnode=10,       # Otro par?metro de complejidad.
              train.fraction=0.8,      # delta
              bag.fraction=0.7,        # eta (Bagging sobre la fracci?n de train)
              cv.folds=5, 
              verbose=T)


gbm.perf(reg.gbm,method="cv")
title(main="Estimaciones de la tasa de error")
legend(500,0.3,c("Error 5 folds CV","Error Test (1-delta)","Error en Train (delta)"),
       col=c('green','red','black'),pch='-', bty = 'n', cex = 0.7)

best.iter = gbm.perf(reg.gbm,method="cv")
summary(reg.gbm,n.trees=best.iter)

m.prec.gmb = predict(reg.gbm,n.trees=best.iter)
plot(log(calif.Train$MedianHouseValue),m.prec.gmb,xlab="Y",
     ylab="Y-hat",main="Pred vs. Obs (in sample)",pch='.')
abline(0,1,lwd=2,col=2) 


### Resultados sobre la muestra test: CAMBIAR POR TE Y AUC
m.prec.gmb = predict(reg.gbm,n.trees=best.iter, newdata=calif.Test)
MSE.for = sum((m.prec.gmb - log(calif.Test$MedianHouseValue))^2)/nrow(calif.Test)
MSE.for 
